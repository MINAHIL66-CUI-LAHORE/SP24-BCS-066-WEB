...
// express-ejs-layouts – disabled by default; sales route opts in via layout option
...
app.use((req, res, next) => {
  ...
  // Query string (used for error/success messages via URL)
  res.locals.query = req.query;

  // ── Cart count: total quantity of all items in the session cart ──────────
  // This makes cartCount available in every EJS template (including header.ejs)
...
app.use('/sales',    require('./routes/sales'));    // GET /sales + GET /api/sales-data
app.use('/cart',     require('./routes/cart'));     // 🆕 Cart (session-based)

// ── 🆕 LAB 4: RESTful API routes ─────────────────────────────────────────────
